import { z } from "zod";
import { BUILDING_TYPES, CITIES, DISTRICTS, SOURCES, STAGES } from "./constants";
...
/** 手動新增 / 採集機推入共用的房源輸入。property 欄位 + 一筆 listing 欄位攤平在一起。 */
export const PropertyInput = z
  ...
  .refine((v) => DISTRICTS[v.city].includes(v.district), { message: "行政區不屬於該縣市", path: ["district"] });
