...
# 移除:  Unregister-ScheduledTask -TaskName "RentHouseDailySync" -Confirm:$false
# 立即測試:  Start-ScheduledTask -TaskName RentHouseDailySync
...
Register-ScheduledTask `
    -TaskName "RentHouseDailySync" `
    -Description "租屋筆記每日 591 採集(雙北)" `
    -Action $action `
    -Trigger $trigger `
    -Settings $settings `
    -Force | Out-Null

Write-Host "已註冊 'RentHouseDailySync'(每天 20:00)。立即測試:Start-ScheduledTask -TaskName RentHouseDailySync"
